...
当前为 `0.1.3-alpha`。已接通主电脑同时连接两台子电脑、快捷键切换、三端文字同步和经主电脑转发的子电脑间文件复制。键鼠/心跳与文字/文件使用两条独立的认证 TLS 连接。三端 Linux X11 CLI 测试已验证切换、文字、目录、紧急退出及子电脑离线时整组退出。自动重连和系统托盘界面已接入；屏幕边缘切换、Wayland 输入 portal、跨连接续传和 Windows/macOS 实体机矩阵仍在实施中。准确状态位于 `specs/MYAPP-001/IMPLEMENTATION.md`。
...
这是配置片段，需要加入已有配置对象。子电脑分别运行 `myapp serve`，主电脑运行 `myapp control`（无需 peer/addr 参数）。默认控制列表中的第一台；按 `Ctrl+Alt+1` / `Ctrl+Alt+2` 切换，按 `Ctrl+Alt+Esc` 退出。三个节点可通过主电脑同步文字及文件；主电脑必须在线。`max_devices` 包含主电脑，最多为 3。任一子会话失败时 controller 会按退避策略重建整组连接；用户主动退出仍会立即结束。
...
原生 GUI 的“设置”页面还可以启用/关闭日志并选择 `debug`、`info`、`warn` 或 `error` 最低级别。日志默认保存到用户配置目录的 `MyApp/logs/myapp.log`，点击“导出日志”会复制全部日志到 `Downloads` 目录；配对码、私钥、剪贴板正文和文件内容不会写入日志。

原生 GUI 的“配对”页面使用监听/连接单选模式，分别显示对应地址配置，并支持终止当前配对子进程；页面会将连接拒绝、网络不可达和超时转换为可操作提示。controller 配对成功后会自动将受控设备加入 `peers`。主电脑和受控电脑都可以通过“启动服务/停止服务”控制本机服务进程。

文件复制完成后会恢复清单中的文件和目录权限、修改时间；接收过程仍先写入受控缓存目录，校验成功后再原子提交。
